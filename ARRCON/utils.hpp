...
public:
	...
	friend std::ostream& operator<<(std::ostream& os, const Help& help)
	{
		return os << help._program_name << ' ' << ((help._program_name != std::string_view(DEFAULT_PROGRAM_NAME)) ? "("s + DEFAULT_PROGRAM_NAME + ") "s : "") << "v" << ARRCON_VERSION << '\n'
			<< "Another RCON Client, compatible with any game using Valve's Source RCON Protocol.\n"
			<< '\n'
			<< "USAGE:" << '\n'
			...
			<< "  -S <host>      --saved <host>      Use a saved host's connection information, if it isn't overridden by arguments." << '\n'
			<< "                 --save-host <H>     Save the target specified with the [-H|-P|-p] options as \"<H>\"" << '\n'
			<< "                 --remove-host <H>   Remove a saved host named \"<H>\" from the list, then exit." << '\n'
			<< "  -l             --list-hosts        Show a list of all currently saved hosts, then exit." << '\n'
			<< '\n'
			<< "OPTIONS:\n"
			<< "  -h         --help                  Show the help display, then exit." << '\n'
			<< "  -v         --version               Print the current version number, then exit." << '\n'
			<< "  -s  -q     --quiet                 Silent/Quiet mode; prevents or minimizes console output." << '\n'
			<< "  -t  -i     --interactive           Starts an interactive command shell after sending any scripted commands." << '\n'
			<< "  -w <ms>    --wait <ms>             Wait for \"<ms>\" milliseconds between sending each command in mode [2]." << '\n'
...
inline void handle_hostfile_arguments(const opt::ParamsAPI2& args, net::HostList& hosts, const std::filesystem::path& hostfile_path)
{
	...
	// list all hosts
	if (args.check_any<opt::Flag, opt::Option>('l', "list-hosts")) {
		do_exit = true;
		if (hosts.empty()) {
